...
| `--tiers TIERS` | all 7 | Comma-separated list of tiers to run, e.g. `T4,T5,T6,T7`. Accuracy is averaged over the tested tiers only. Useful for focusing on hard probes against local/quantized models. |
...
Local quantized models (GGUF, AWQ, GPTQ, etc.) can be tested against
the harder tiers only, skipping the trivial T1–T3 probes that every
model above 7B answers correctly.
...
The judge (`google/gemini-3-flash-preview`) always runs on OpenRouter,
so `OPENROUTER_API_KEY` is required even for fully local targets.
...
T4–T7 contains 800 probes total (200 per tier). Add `--sample 280` to
run 70 per tier instead — roughly 3× faster, still useful for
ranking:
...
> The model name must match the identifier registered in the inference
> server — vLLM uses `--served-model-name`; Ollama uses the model tag.

### Quick preview before committing

Inspect T4–T7 sample questions before spending tokens:

```bash
python scripts/ikp_estimate.py --inspect-probes | grep -A4 "── T4 ──" | head -20
```

Run a tiny sample with full per-probe output to sanity-check:
...
When `--tiers` is used, the displayed parameter estimate is the
calibration curve applied to the **mean of the tested tiers only**
(not a full 7-tier average). This makes the absolute number
incomparable to the paper's reported sizes — use it for **relative
ranking** across your own model variants, not as a ground-truth size
claim.
