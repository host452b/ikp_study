...
This is the ikp_study overlay version. It lives at ikp_study/scripts/ and
automatically resolves data paths into the ikp/ submodule.

Additions over the upstream ikp/scripts/ikp_estimate.py:
  --tiers  Comma-separated tier filter, e.g. T4,T5,T6,T7

Usage:
  # Run from ikp_study/ root
...
# ── Constants ──────────────────────────────────────────────────
# Point into the ikp/ submodule so data files are found correctly
# when this script is run from ikp_study/scripts/.
...
def display_results(model_name: str, results: list, tier_accs: dict, accuracy: float,
    ...
    # Confidence note
    active_tiers = [t for t in ["T1","T2","T3","T4","T5","T6","T7"]
                    if any(r["tier"] == t for r in results)]
    if active_tiers != ["T1","T2","T3","T4","T5","T6","T7"]:
        print(f"\n  {DIM}⚠ Partial run: only tiers {', '.join(active_tiers)}. "
              f"Estimate is averaged over tested tiers only (not globally calibrated).{RESET}")
...
def main():
    ...
    eval_group.add_argument("--tiers", metavar="TIERS",
                            help="Comma-separated tiers to run, e.g. T4,T5,T6,T7 (default: all)")
    ...
    # Only average over tiers that were actually tested
